...
- **연구 기반 근본 원인 태깅** ⭐ 신규
  - 각 검증 문제를 연구 기반 근본 원인(RC01-RC07)에 연결
  - Kim & Chi (2020), Hwang et al. (2023) 한국 건설 안전 연구 기반
  - 분류: 교육 부족, 보호구 미제공, 안전조치 미이행, 불안전한 상태, 안전 불감증, 절차 이탈, 안전계획 부재
  - 증거 기반 분석 및 교차 소스 종합 지원
...
2. **위험도 행렬 계산**
   ...
   - **추락 위험 우선순위 시스템** ⭐ 신규
     - 건설업 사망사고의 71%가 추락사고라는 연구 결과 기반
     - 한국어/영어 키워드를 사용한 자동 고소작업 감지 (비계, 고소작업, scaffold 등)
     - 고소작업 감지 시 추락 관련 문제를 "오류" 심각도로 격상
     - 교차 소스 감지: TBM 언급 + 문서 내용 + 사진 분석
...
### 🎤 TBM (Tool Box Meeting) 녹음 및 분석 ⭐ 강화
...
#### **참여도 품질 점수** ⭐ 신규
- 참여형 TBM이 더 효과적이라는 PMC 연구 기반 **연구 기반 분석**
- **5가지 참여 요인 분석**:
  - 질문 여부 (토론 vs 강의 판별)
  - 다자 참여 감지 (화자 전환 패턴)
  - 작업자 참여 표현 (확인, 응답)
  - 적정 소요시간 (단어 수 분석)
  - 개인별 호명 (이름 언급으로 참여 유도)
- **점수 등급**: 높음/참여형 (75+), 보통 (50-74), 낮음/일방적 전달 (0-49)
- 낮은 참여도 TBM에 대한 개선 제안 생성
- TBM 타임라인에 색상 코드 배지로 표시
- 요인 분석과 함께 PDF 내보내기에 포함

#### **AI 기반 TBM 교차 검증**
- 체크리스트를 TBM 논의 내용과 비교 검증
- TBM에서 언급된 위험요인이 문서에서 체크되지 않은 경우 감지
- Claude Sonnet 4.5와 GPT-5.1 폴백을 사용한 의미 매칭
- **추락 위험 우선순위**: TBM에서 고소작업 감지 시 자동 심각도 격상
...
├── rootCauseMapping.ts         # 연구 기반 근본 원인 분류 (신규)
├── fallHazardPriority.ts       # 추락 위험 감지 및 격상 (신규)
└── tbmEngagementScoring.ts     # TBM 참여도 품질 분석 (신규)
```
...
### 규칙 범위
- **Stage 1**: 5개 형식 검사
- **Stage 2**: 22개 논리 규칙 (4개 카테고리) + 근본 원인 태깅
- **Stage 3a**: 8개 구조화된 검증 함수
- **Stage 3b**: 4가지 요인 위험 평가 + 추락 위험 우선순위
- **Stage 3c**: 3개 문서 간 분석
- **Stage 3d**: TBM + 사진 교차 검증 (AI 기반)
- **Stage 4**: 5개 패턴 감지 알고리즘
- **Stage 5**: 상황별 안전 검토 (6개 우려 카테고리)
- **TBM**: 7개 완전성 기준 + 5개 참여도 요인

**총**: 5단계에 걸쳐 55개 이상의 검증 규칙

### 연구 통합
- **근본 원인**: 7개 연구 기반 분류 (RC01-RC07)
- **추락 우선순위**: 71% 사망사고 통계 기반
- **참여도 점수**: PMC 참여형 교육 연구 기반

### 코드 지표
- **총 라인 수**: ~4,000줄 (검증 엔진 + TBM + PDF 내보내기)
- **모듈**: 15개 주요 검증 파일
...
11. **연구 기반 근본 원인** ⭐ 신규 - 학술 연구 연결 (Kim & Chi 2020, Hwang et al. 2023)
12. **추락 위험 우선순위** ⭐ 신규 - 71% 사망사고 연구 기반 자동 격상
13. **TBM 참여도 점수** ⭐ 신규 - 일방적 전달 vs 참여형 미팅 감지
